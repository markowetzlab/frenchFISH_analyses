...
###
processFISH <- function(combinedImg, writedir, bgCorrMethod=list(1,100), channelSignals=NULL, channelColours=NULL,
                        sizeNucleus = c(5,15000), sizeProbe =c(5,100), gaussigma=20,outputImageFormat=".png",norm_method,
                        Norm_intensity){      
  starttime <- Sys.time()
  ...
  for(im in 1:length(channelImg)){
    ...
    #geoff's edits
    imgG = processChannels(imgG, sizeProbe[im,2], sizeProbe[im,1],norm_method[im],Norm_intensity,CellMask)
    channelImg[[im]] <- imgG
...
#
###
processChannels <- function(Image, Maxsize, Minsize, norm_method, Norm_intensity,CellMask){  
  #Method<-"RenyiEntropy"
  # if(norm_method[1])
  # {
  # #remove bright, saturated spots
  # pixel_matrix<-as.numeric(Image)
  # dim(pixel_matrix)<-dim(Image)
  # raster_matrix<-raster(pixel_matrix)
  # adj_matrix<-matrix(ncol=80,nrow=80,data = 1)
  # adj_matrix[40,40]<-0
  # saturated_cells<-which(values(raster_matrix==1))
  # neighbourhood<-adjacent(raster_matrix,saturated_cells,adj_matrix,pairs=FALSE)
  # raster_matrix[neighbourhood]<-0
  # raster_matrix[saturated_cells]<-0
  # Image@.Data<-as.matrix(raster_matrix)
  # #display(Image)
  # }
  
  #mask nuclei
  Image<-imageSubtract(Image,1-CellMask)
  
  #remove autoflourescence
  non_zero_image<-Image[Image>0]
  norm_thresh<-sort(as.numeric(non_zero_image))[0.90*length(as.numeric(non_zero_image))]
  #norm_thresh<-2*mean(Image)
  #if(norm_thresh>0.9){norm_thresh<-0.9}
  Image<-normalize(Image,inputRange=c(norm_thresh,max(Image)))
  
  #if(norm_method[2])#remove autoflourescence
  # {
  #   m<-max(kmeans(as.numeric(Image),2)$centers) #fits two gaussians to attempt to remove background signal
  #   if((Norm_intensity-m)<0)
  #   {
  #     Image<-Image+(Norm_intensity-m)
  #     Image[Image>1]<-1
  #     Image[Image<0]<-0
  #   }
  # }
  #if(norm_method[2])#remove precipitation
  #{
  #  Method="Intermodes"
  #}
  #else{
  ...
  Image<-gblur(Image, sigma=2)
  t=auto_thresh(as.integer(as.numeric(Image)*255),Method,ignore_black = TRUE)[1]/255
...
writeArguments <- function(bgCorrMethod,channelColours,channelSignals,sizeNucleus, sizeProbe){
  ...
  string1 = c(string1,paste('Maximum area', sizeProbe[1,1], sep=" - "), paste('Minimum area', sizeProbe[1,2], sep=" - "))
...
checkArguments <- function (writedir,channelSignals,channelColours,sizeNucleus,sizeProbe){
  ...
  if(file.exists(writedir)){
    ...
    if(length(sizeNucleus)!=2 || class(sizeNucleus)!="numeric" || ncol(sizeProbe)!=2 || class(c(sizeProbe))!="numeric"){
...
GetNucleus <- function(Image,iCell,Features){
  ...
  if (OriginX+Height>dim(Image)[1]){
    Height<-round(Height-((OriginX+Height)-dim(Image)[1]))
  }
  if (OriginY+Width>dim(Image)[2]){
    Width<-round(Width-((OriginY+Width)-dim(Image)[2]))
  }
